...
def FxForMPPFairgroup3(X, sources, g1, g2, g3, p):
    resultG1 = {}
    resultG2 = {}
    resultG3 = {}  
    g1Dict = {}
    ...
    for node in length:
        if node in g1Dict:
            ...
            resultG3[node] = max(resultG3[node] if node in resultG3 else 0, p**(length[node]))
    
    resg1 = sum(resultG1[x] for x in resultG1) / len(g1)
    resg2 = sum(resultG2[x] for x in resultG2) / len(g2)
    resg3 = sum(resultG3[x] for x in resultG3) / len(g3) 
    
    return resg1, resg2, resg3  
...
def FxForMPPFairNewEdgesgroup3(X, shortestDistances, newEdges, g1, g2, g3, resg1, resg2, resg3, p):
    updatedLength = shortestDistancesNewEdges(X, newEdges, shortestDistances)
    ...
    for node in updatedLength:
        if node in g1Dict:
            ...
            resg3 = resg3 + ((p**(updatedLength[node]) - (p**(shortestDistances[node]) if node in shortestDistances else 0)) / len(g3))
